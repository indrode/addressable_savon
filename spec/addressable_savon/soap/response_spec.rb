...
describe AddressableSavon::SOAP::Response do

  describe ".new" do
    it "raises a AddressableSavon::SOAP::Fault in case of a SOAP fault" do
      lambda { soap_fault_response }.should raise_error(AddressableSavon::SOAP::Fault)
    end

    it "does not raise a AddressableSavon::SOAP::Fault in case the default is turned off" do
      AddressableSavon.raise_errors = false
      lambda { soap_fault_response }.should_not raise_error(AddressableSavon::SOAP::Fault)
      AddressableSavon.raise_errors = true
    end

    it "raises a AddressableSavon::HTTP::Error in case of an HTTP error" do
      lambda { soap_response :code => 500 }.should raise_error(AddressableSavon::HTTP::Error)
    end

    it "does not raise a AddressableSavon::HTTP::Error in case the default is turned off" do
      AddressableSavon.raise_errors = false
  ...
  describe "#soap_fault" do
    ...
    it "returns a AddressableSavon::SOAP::Fault" do
    ...
    it "returns a AddressableSavon::SOAP::Fault even if the SOAP response seems to be ok" do
      soap_response.soap_fault.should be_a(AddressableSavon::SOAP::Fault)
    end
  end
  ...
  describe "#http_error" do
    ...
    it "returns a AddressableSavon::HTTP::Error" do
    ...
    it "returns a AddressableSavon::HTTP::Error even if the HTTP response seems to be ok" do
      soap_response.http_error.should be_a(AddressableSavon::HTTP::Error)
    end
  end
