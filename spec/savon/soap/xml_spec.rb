...
describe AddressableSavon::SOAP::XML do
  let :xml do
    xml = AddressableSavon::SOAP::XML.new
  ...
  describe "#header" do
    ...
    it "uses the global soap_header if set" do
      AddressableSavon.stubs(:soap_header).returns({ "MySecret" => "abc" })
      xml.header.should == { "MySecret" => "abc" }
    end
  end
  ...
  describe "#env_namespace" do
    ...
    it "uses the global env_namespace if set as the SOAP envelope namespace" do
      AddressableSavon.stubs(:env_namespace).returns(:soapenv)
      xml.env_namespace.should == :soapenv
    end
  end
  ...
  describe "#wsse" do
    it "sets the AddressableSavon::WSSE object" do
      xml.wsse = AddressableSavon::WSSE.new
      xml.wsse.should be_a(AddressableSavon::WSSE)
    end
  end
  ...
  def reset_soap_version
    AddressableSavon.soap_version = AddressableSavon::SOAP::DefaultVersion
  end
